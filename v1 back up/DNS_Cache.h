...
struct CacheEntry
{
    ...
    size_t                     ip_len;
    TimePoint                  expiry;
    bool                       chance{true}; // 不可拷贝且不可移动对象，所以需要显示实现拷贝构造函数

    CacheEntry() = default;

    CacheEntry(std::string d, std::vector<unsigned char> i, size_t i_l, TimePoint e)
        : domain(std::move(d))
        , ip(std::move(i))
        , ip_len(i_l)
        , expiry(e)
...
public:
    ...
    std::optional<std::vector<unsigned char>> get(const std::string &domain, const TimePoint &now);

    void put(const std::string &domain, const std::vector<unsigned char> &ip, const TimePoint &now);
