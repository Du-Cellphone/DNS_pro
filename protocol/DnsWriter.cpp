...
WriteResult make_error_response(const Message &request, ResponseCode response_code, bool recursion_available, size_t maximum_size)
{
    ...
    auto response = write_question_message(response_header, request.questions, maximum_size);
    if (response || response.error().code != WriteErrorCode::MessageTooLarge)
        return response;

    // A local error is still useful when echoing the question would exceed the
    // classic UDP response budget. Never return a partially written question.
...
WriteResult make_header_only_error_response(std::span<const std::byte> request_prefix,
                                            ...
                                            size_t                    maximum_size)
{
    if (request_prefix.size() < sizeof(uint16_t))
        return write_failure(WriteErrorCode::MissingTransactionId);

    Header response_header;
    response_header.id = static_cast<uint16_t>((static_cast<uint16_t>(std::to_integer<uint8_t>(request_prefix[0])) << 8U) |
                                               static_cast<uint16_t>(std::to_integer<uint8_t>(request_prefix[1])));
    response_header.is_response         = true;
    response_header.recursion_available = recursion_available;
    response_header.response_code       = static_cast<uint8_t>(response_code);

    if (request_prefix.size() >= 3)
    {
        const uint8_t high_flags = std::to_integer<uint8_t>(request_prefix[2]);
    ...
    if (request_prefix.size() >= 4)
    {
        const uint8_t low_flags = std::to_integer<uint8_t>(request_prefix[3]);
...
{
    return make_header_only_error_response(malformed_request, ResponseCode::FormErr, recursion_available, maximum_size);
}
